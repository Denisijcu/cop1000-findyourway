...
const mikyThink = document.querySelector("#mikyThink");
const lab = document.querySelector("#lab");
const cors = document.querySelector("#cors");
const btn = document.querySelector(".btn");
const h1 = document.querySelector(".find");
...
function start(status) {
  h1.style.size = "14px";
  //Show images
  //console.log("ES FALSO", status);
  if (status) {
    mikyThink.style.display = "none";
  }
  ...
  if (btn.textContent === "Init") {
    status = false;
    mikyThink.style.display = "inline-block";
  }
  s = "";
  for (let i = 0; i < matriz.length; i++) {
    s += "<tr>";
    for (let j = 0; j < matriz.length; j++) {
      // setTimeout(() => stop(i), 1000);
  ...
  var classImage = status ? "showImage" : "hideImage";
  var data =
    `
    ...
    `</table>
    <br />
    <img src="miky.jpg" alt="Miky" id="mikyGotIt" class='${classImage}'>
    `;
  ...
  var t = "";
  coorXY.forEach(c => {
    t += `<tr><td class= 'tableCoors'>${c.x}</td><td class= 'tableCoors'>${
      c.y
    }</td></tr>`;
  });
  if (!status) {
  ...
  var tableCoors =
    `<h2>COORS </h2><table style="width:50%"> <th> X </th> <th> Y </th>` +
    t +
    `</table>
...
setInterval(() => start(run), 55000);

/*
function stop(i) {
  console.log("el varlo de i", i);
}
*/
